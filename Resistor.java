...
class Resistor {
  ...
  private void initColorArrays() {
    ...
    tolerance = new String[] { "gold", "silver" };
  }
  ...
  public double calculateResistance(String[] colors) {
    double resistance = Double.parseDouble(
      colorCode.get(colors[0]) + colorCode.get(colors[1])
    );
  ...
  public String formatResistance(double resistance) {
    ...
    if (resistance >= 1000000) {
      ...
      formattedResistance =
        String.format((resistance % 1 == 0) ? "%.0f" : "%.2f", resistance);
    }
